...
#### 왜 이 영역을 Terraform으로 관리했는가

실제 운영에서는 모든 인프라를 Terraform으로 옮기기보다, 반복적으로 변경되는 지점부터 우선 정리했습니다. 그중 가장 불편했던 부분이 CodeBuild였습니다.

- 신규 앱 추가 시 CodeBuild 프로젝트를 계속 새로 만들어야 했음
- 서비스마다 Source Repository, Branch, ECR Repository, Manifest 경로가 달랐음
- 수동으로 관리하면 환경별 설정 누락이나 오타 가능성이 높았음

그래서 VPC나 EKS처럼 "한 번 구축 후 비교적 안정적으로 유지되는 영역"보다, 서비스 수에 따라 계속 늘어나는 CodeBuild를 먼저 Terraform 모듈과 변수 파일로 관리했습니다.

여기서 중요한 점은, 이 프로젝트의 Terraform 경험은 CI 리소스 관리에 집중되어 있었다는 것입니다. VPC나 EKS를 Terraform으로 일괄 구축한 케이스로 읽히지 않도록, 공개용 샘플과 실무 적용 범위는 구분해서 보는 것이 맞습니다.
...
- 신규 서비스 추가 시 CodeBuild 프로젝트 생성을 반복하지 않고 변수 추가 중심으로 관리 가능
