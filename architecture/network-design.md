...
- CloudFront 앞단에서 WAF 정책으로 국가, IP, 요청 패턴 일부를 선제적으로 제어
- Kubernetes 내부 라우팅은 Ingress와 Service로 처리
- Pod 자체는 외부에 직접 노출되지 않음

#### 운영 포인트

- `prod`는 공용 서비스 특성상 CloudFront + WAF 기반 보호를 전제로 운영
- `dev / qa`는 허용된 IP나 제한된 접근 정책 중심으로 외부 노출 범위를 축소
- 즉, 네트워크 구조 자체와 별도로 엣지 구간 접근 제어까지 함께 고려한 설계였습니다.
...
- CloudFront / WAF 정책을 환경별로 다르게 가져갈 때의 운영 복잡도
