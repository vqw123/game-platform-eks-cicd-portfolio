...
## CloudFront and WAF Controls

### 왜 WAF를 함께 사용했는가

외부 요청이 ALB와 Kubernetes까지 도달하기 전에, 엣지 구간에서 먼저 걸러내는 것이 운영상 더 효율적이었습니다.

실제 운영에서는 CloudFront 앞단에 AWS WAF를 연결해, 단순 네트워크 노출만 줄이는 것이 아니라 "어떤 요청을 애초에 받지 않을 것인가"까지 제어했습니다.
...
- Bot / User-Agent 계열 탐지 규칙 적용
- 네트워크 단 IP 기반 접근 제한
- `dev / qa` 환경에 대한 제한적 접근 정책

### 운영 의도

- 운영 환경과 비운영 환경의 외부 노출 수준을 다르게 가져가기 위함
- 비정상적이거나 불필요한 요청을 애플리케이션 레벨까지 전달하지 않기 위함
- 공용 진입점 보안을 CloudFront / WAF 레벨에서 먼저 정리하기 위함

### 환경별 적용 관점

- `prod`
  - 일반 사용자 요청을 받는 전제에서 Rate limit, Managed Rules, Bot 관련 방어를 우선 고려
- `dev / qa`
  - 테스트 목적 환경이므로 허용된 IP 중심 접근 제어를 더 강하게 적용

### 실무적인 의미

이 경험은 단순히 "WAF를 붙였다"보다, 서비스 운영 환경과 테스트 환경에 따라 외부 노출 정책을 다르게 운영해 본 경험에 가깝습니다. 즉, Kubernetes 내부 보안뿐 아니라 CloudFront 앞단의 엣지 보안 운영도 함께 다뤘다는 점이 중요했습니다.
...
이 프로젝트에서는 운영 가능하면서도 안전한 구조를 우선 확보하고, 이후 정교화할 수 있는 형태로 구성했습니다. 보안도 Security Group과 IAM만의 문제가 아니라, CloudFront / WAF / Bastion / IRSA / 데이터 계층 접근 제어가 함께 맞물리는 운영 문제로 봤습니다.
