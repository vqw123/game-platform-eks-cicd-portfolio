...
실제 게임/백엔드 서비스의 EKS 구축 및 운영 경험을 바탕으로, 인프라 구조·배포 체계·운영 기준을 공개 가능한 형태로 재구성한 포트폴리오입니다.

네트워크 분리, 접근 제어, GitOps 기반 배포, 오토스케일링, 서비스 계정 권한 분리, 로그 및 메트릭 수집 체계를 실제 운영 관점에서 정리했으며, `dev / qa / prod` 환경 분리와 `CodeBuild`, `ECR`, `ArgoCD` 기반 배포 흐름도 함께 문서화했습니다.

모든 인프라를 처음부터 Terraform으로 관리한 것은 아니며, 반복 작업이 많았던 `CodeBuild` 영역을 우선 Terraform으로 관리했습니다. 반면 VPC, EKS, Add-on 등은 당시 운영 우선순위에 맞춰 수동 구성과 문서화를 병행했습니다.
...
- CloudFront + AWS WAF 기반 엣지 레벨 접근 제어 운영
- 운영 절차 문서화 및 표준화
...
- CloudFront + AWS WAF 기반 엣지 보안 적용
...
- 노션 기반 운영 문서 정리를 통한 팀 내 재사용성 확보
...
- `terraform/`
  - 실제 적용 코드가 아닌, 아키텍처 설명용 Terraform 예시
- `argocd/`
  - ArgoCD 설정 예시
- `k8s/`
  - Kubernetes Manifest 예시
- `codebuild/`
  - 실제 Terraform 적용 경험과 연결되는 CodeBuild 예시
...
특히 Terraform은 반복적으로 추가·수정이 필요했던 CodeBuild 프로젝트 관리에 가장 먼저 적용했습니다. 신규 앱이나 서비스가 생길 때마다 Source Repository, 브랜치, ECR 대상, Manifest 경로가 달라지는 CodeBuild 구성이 계속 늘어났고, 이 지점을 변수 기반으로 정리하는 편이 운영 효율에 가장 직접적인 효과가 있었습니다.

반대로 VPC, EKS, Add-on 구성은 당시 실제 운영에서 Terraform으로 관리하지 않았습니다. 이 저장소의 `infra-sample/terraform`은 실무 적용 코드가 아니라, 네트워크 구조를 공개용으로 설명하기 위해 별도로 만든 예시입니다.
...
- AWS WAF 기반 국가/IP/rate limit 접근 제어 운영
...
- CloudFront와 WAF를 활용한 엣지 보안 운영 방식
