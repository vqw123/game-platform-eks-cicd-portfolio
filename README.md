...
특히 모든 인프라를 처음부터 Terraform으로 관리한 것은 아니며, 실제 운영에서는 변경 빈도가 높고 서비스 추가 시 반복 작업이 많이 발생하던 `CodeBuild` 영역을 우선 Terraform으로 관리했습니다. 반면 VPC, EKS, Add-on 등은 당시 운영 우선순위와 일정에 맞춰 수동 구성과 문서화를 병행했습니다.
